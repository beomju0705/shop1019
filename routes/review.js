...
router.get('/list.json', function(req, res){  //테스트 http://localhost:3000/review/list.json?bid=132&page=1
    const bid=req.query.bid;
    const page=parseInt(req.query.page);
    const start=(page-1)*5;
    const sql='select * from view_review where bid=132 limit 0,5';
    db.get().query(sql, [bid, start], function(err, rows){
        res.send(rows);
    });
});

router.get("/count", function(req, res){ //테스트 http://localhost:3000/review/count?bid=118
    const bid=req.query.bid;
